...
#pragma warning disable SYSLIB5001 // Type is for evaluation purposes only and is subject to change or removal in future updates. Suppress this diagnostic to proceed.
        ...
        public static void Part1(TextWriter writer) {
            ...
            // 
            // 标量
            // 如果你曾经在餐厅支付餐费，那么应该已经知道一些基本的线性代数，比如在数字间相加或相乘。 例如，北京的温度为 52∘𝐹
            //  （华氏度，除摄氏度外的另一种温度计量单位）。 严格来说，仅包含一个数值被称为标量（scalar）。 如果要将此华氏度值转换为更常用的摄氏度， 则可以计算表达式 𝑐=59(𝑓−32)
            //  ，并将 𝑓
            //  赋为 52
            //  。 在此等式中，每一项（ 5
            //  、 9
            ...
            // 
            // 本书采用了数学表示法，其中标量变量由普通小写字母表示（例如， 𝑥
            //  、 𝑦
            //  和 𝑧
            //  ）。 本书用 ℝ
            //  表示所有（连续）实数标量的空间，之后将严格定义空间（space）是什么， 但现在只要记住表达式 𝑥∈ℝ
            //  是表示 𝑥
            //  是一个实值标量的正式形式。 符号 ∈
            //  称为“属于”，它表示“是集合中的成员”。 例如 𝑥,𝑦∈{0,1}
            //  可以用来表明 𝑥
            //  和 𝑦
            //  是值只能为 0
            //  或 1
            ...
            // 来引用第𝑖
            // 个元素。 注意，元素𝑥𝑖
            // 是一个标量，所以我们在引用它时不会加粗。 大量文献认为列向量是向量的默认方向，在本书中也是如此。 在数学中，向量𝐱
            // 可以写为：
            // 
            // 𝐱=𝑥1𝑥2⋮𝑥𝑛,
            // :eqlabel:eq_vec_def
            ...
            // 
            // 矩阵
            // 正如向量将标量从零阶推广到一阶，矩阵将向量从一阶推广到二阶。 矩阵，我们通常用粗体、大写字母来表示 （例如，𝐗
            // 、𝐘
            // 和𝐙
            // ）， 在代码中表示为具有两个轴的张量。
            // 
            ...
            // ，其由𝑚
            // 行和𝑛
            // 列的实值标量组成。 我们可以将任意矩阵𝐀∈ℝ𝑚×𝑛
            // 视为一个表格， 其中每个元素𝑎𝑖𝑗
            // 属于第𝑖
            // 行第𝑗
            // 列：
            // 
            // 𝐀=𝑎11𝑎21⋮𝑎𝑚1𝑎12𝑎22⋮𝑎𝑚2⋯⋯⋱⋯𝑎1𝑛𝑎2𝑛⋮𝑎𝑚𝑛.
            ...
            // 的小写字母索引下标𝑎𝑖𝑗
            //  来引用[𝐀]𝑖𝑗
            // 。 为了表示起来简单，只有在必要时才会将逗号插入到单独的索引中， 例如𝑎2,3𝑗
            // 和[𝐀]2𝑖−1,3
            // 。
            // 
            // 当我们交换矩阵的行和列时，结果称为矩阵的转置（transpose）。 通常用𝐚⊤
            // 来表示矩阵的转置，如果𝐁=𝐀⊤
            // ， 则对于任意𝑖
            // 和𝑗
            // ，都有𝑏𝑖𝑗=𝑎𝑗𝑖
            // 。 因此，在 :eqref:eq_matrix_def中的转置是一个形状为𝑛×𝑚
            // 的矩阵：
            // 
            // 𝐀⊤=𝑎11𝑎12⋮𝑎1𝑛𝑎21𝑎22⋮𝑎2𝑛……⋱…𝑎𝑚1𝑎𝑚2⋮𝑎𝑚𝑛.
            // 
            // 现在在代码中访问(矩阵的转置)。
            // 
            // A.T
        ...
        public static void Part2(TextWriter writer) {
            ...
            // 
            // 我们可以对任意张量进行的一个有用的操作是[计算其元素的和]。 数学表示法使用∑
            // 符号表示求和。 为了表示长度为𝑑
            // 的向量中元素的总和，可以记为∑𝑑𝑖=1𝑥𝑖
            // 。 在代码中可以调用计算求和的函数：
            // 
            ...
            // 中的值根据权重𝐰
            // 的加权和， 可以表示为点积𝐱⊤𝐰
            // 。 当权重为非负数且和为1（即(∑𝑑𝑖=1𝑤𝑖=1)
            // ）时， 点积表示加权平均（weighted average）。 将两个向量规范化得到单位长度后，点积表示它们夹角的余弦。 本节后面的内容将正式介绍长度（length）的概念。
            // 
            // 矩阵-向量积
            // 现在我们知道如何计算点积，可以开始理解矩阵-向量积（matrix-vector product）。 回顾分别在 :eqref:eq_matrix_def和 :eqref:eq_vec_def中定义的矩阵𝐀∈ℝ𝑚×𝑛
            // 和向量𝐱∈ℝ𝑛
            // 。 让我们将矩阵𝐀
            // 用它的行向量表示：
            // 
            ...
            // ：
            // 
            // 𝐀=𝑎11𝑎21⋮𝑎𝑛1𝑎12𝑎22⋮𝑎𝑛2⋯⋯⋱⋯𝑎1𝑘𝑎2𝑘⋮𝑎𝑛𝑘,𝐁=𝑏11𝑏21⋮𝑏𝑘1𝑏12𝑏22⋮𝑏𝑘2⋯⋯⋱⋯𝑏1𝑚𝑏2𝑚⋮𝑏𝑘𝑚.
            // 
            // 用行向量𝐚⊤𝑖∈ℝ𝑘
            // 表示矩阵𝐀
            // 的第𝑖
            ...
            // 的列向量:
            // 
            // 𝐀=𝐚⊤1𝐚⊤2⋮𝐚⊤𝑛,𝐁=[𝐛1𝐛2⋯𝐛𝑚].
            // 当我们简单地将每个元素𝑐𝑖𝑗
            // 计算为点积𝐚⊤𝑖𝐛𝑗
            // :
            // 
            // 𝐂=𝐀𝐁=𝐚⊤1𝐚⊤2⋮𝐚⊤𝑛[𝐛1𝐛2⋯𝐛𝑚]=𝐚⊤1𝐛1𝐚⊤2𝐛1⋮𝐚⊤𝑛𝐛1𝐚⊤1𝐛2𝐚⊤2𝐛2⋮𝐚⊤𝑛𝐛2⋯⋯⋱⋯𝐚⊤1𝐛𝑚𝐚⊤2𝐛𝑚⋮𝐚⊤𝑛𝐛𝑚.
            // 
            // [我们可以将矩阵-矩阵乘法𝐀𝐁
            // 看作简单地执行𝑚
            // 次矩阵-向量积，并将结果拼接在一起，形成一个𝑛×𝑚
            ...
            // 
            // 在线性代数中，向量范数是将向量映射到标量的函数𝑓
            // 。 给定任意向量𝐱
            // ，向量范数要满足一些属性。 第一个性质是：如果我们按常数因子𝛼
            ...
            // ∀𝑖,[𝐱]𝑖=0⇔𝑓(𝐱)=0.
            // 
            // 范数听起来很像距离的度量。 欧几里得距离和毕达哥拉斯定理中的非负性概念和三角不等式可能会给出一些启发。 事实上，欧几里得距离是一个𝐿2
            // 范数： 假设𝑛
            // 维向量𝐱
            // 中的元素是𝑥1,…,𝑥𝑛
            // ，其[**𝐿2
            ...
            // 类似于向量的𝐿2
            // 范数，[矩阵]𝐗∈ℝ𝑚×𝑛
            // (的Frobenius范数（Frobenius norm）是矩阵元素平方和的平方根：)
            // 
            // (
            // ‖𝐗‖𝐹=∑𝑖=1𝑚∑𝑗=1𝑛𝑥2𝑖𝑗⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯.
            // )
            // 
            // Frobenius范数满足向量范数的所有性质，它就像是矩阵形向量的𝐿2
